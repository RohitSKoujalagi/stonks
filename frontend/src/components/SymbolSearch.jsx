      ...
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          <button onClick={() => setExcOpen(o => !o)}
            className="flex items-center gap-1 px-3 py-2 rounded bg-[#1a1d26] border border-white/10
              text-xs font-display text-[#f5c518] hover:border-[#f5c518]/40 transition-colors min-w-[64px]">
            ...
            <div className="absolute top-full mt-1 left-0 bg-[#1a1d26] border border-white/10 rounded shadow-xl z-50 w-24">
              {EXCHANGES.map(ex => (
                <button key={ex} onClick={() => handleExchange(ex)}
                  className={`block w-full text-left px-3 py-2 text-xs font-display hover:bg-white/5 transition-colors
                    ${ex === exchange ? 'text-[#f5c518]' : 'text-[#f5f0e8]'}`}>
                  {ex}
                </button>
              ))}
            </div>
        ...
        <div className="relative flex-1">
          <div className={`flex items-center gap-2 px-3 py-2 rounded border transition-colors
            ${warn ? 'border-[#f5c518]/40 bg-[#f5c518]/5' : 'border-white/10 bg-[#1a1d26] focus-within:border-[#00c896]/50'}`}>
            ...
            <button onClick={() => submit()}
              className="text-[10px] px-2 py-0.5 rounded bg-[#00c896]/10 text-[#00c896]
                hover:bg-[#00c896]/20 transition-colors font-display shrink-0">
              GO
            </button>
          </div>

          {showPop && popular.length > 0 && (
            <div className="absolute top-full mt-1 left-0 right-0 bg-[#1a1d26] border border-white/10
              rounded shadow-xl z-50 max-h-48 overflow-y-auto">
              <p className="px-3 py-1.5 text-[10px] text-[#8892a4] uppercase tracking-widest
                border-b border-white/5 sticky top-0 bg-[#1a1d26]">
                Popular on {exchange}
              </p>
              {popular.map(sym => (
                <button key={sym} onClick={() => { setInput(sym); setWarn(null); submit(sym) }}
                  className="block w-full text-left px-3 py-2 text-xs font-display
                    hover:bg-white/5 text-[#f5f0e8] hover:text-[#00c896] transition-colors">
                  {sym}
                </button>
              ))}
            </div>
