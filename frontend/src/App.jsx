    ...
    <div className="min-h-screen flex flex-col bg-[#0d0f14]">
      ...
      <main className="flex-1 p-4 grid grid-cols-[1fr_288px] gap-4 max-w-[1600px] mx-auto w-full">
        ...
        <div className="flex flex-col gap-4 min-w-0">
          ...
          {/* Tabs: Portfolio / Trade History */}
          <div className="rounded border border-white/5 bg-[#0d0f14] overflow-hidden">
            <div className="flex border-b border-white/5">
              {TABS.map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)}
                  className={`px-5 py-2.5 text-xs font-display uppercase tracking-widest transition-colors ${activeTab === tab ? 'text-[#f5f0e8] border-b-2 border-[#00c896]' : 'text-[#8892a4] hover:text-[#f5f0e8]'}`}>
                  {tab}
